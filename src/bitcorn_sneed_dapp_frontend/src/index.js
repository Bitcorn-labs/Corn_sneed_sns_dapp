import { bitcorn_sneed_dapp_backend } from "../../declarations/bitcorn_sneed_dapp_backend";
...
document.getElementById("convert").addEventListener("click", async (e) => {
  ...
  document.getElementById("result").innerHTML =
    "<img src='loading-gif.gif' class='loading-gif' />";

  const result = await bitcorn_sneed_dapp_backend.convert_account(account);
...
document.querySelector("form").addEventListener("submit", async (e) => {
  ...
  const result = await bitcorn_sneed_dapp_backend.get_account(account);
  ...
  if (ok) {
    ...
    document.getElementById(
      "balance"
    ).innerHTML = `Old(non-SNS) BITCORN : ${toJsonString(balance * 10000)}
        <br>
                                                     New(SNS) BITCORN(Convertable) : ${toJsonString(
                                                       balance
                                                     )}`;
  } else {
...
let status = await bitcorn_sneed_dapp_backend.get_status();
...
if (active) {
  ...
  document.getElementById("test_version").innerText =
    "(Test version original app will be deployed after sns launch)";
}
